...
# 4. Testing Workflow for 25ml
def test_25ml_models(test_data_path, model_dir='soil_nutrient_models'):
    """Complete testing workflow for 25ml water level"""
    ...
    # Filter data for 25ml
    test_data_25ml = test_data[test_data['Records'].str.contains(water_level, na=False)]
    if test_data_25ml.empty:
        raise ValueError(f"No test data available for {water_level} water level")
    
    # Make predictions
    print(f"\nMaking predictions for {water_level} water level...")
    predictions = predict_soil_nutrients(test_data_25ml, models, scalers, wavelengths)
    ...
    for nutrient, pred in predictions.items():
        ...
        y_true = test_data_25ml[true_col]
        y_pred = pred
...
if __name__ == "__main__":
    # Configuration
    TEST_DATA_PATH = r"E:\Nirma hackthon\training\Soil_25ml.csv"
    MODEL_DIR = r"E:\Nirma hackthon\training\soil_nutrient_models"
    
    try:
        print("Starting 25ml water level model testing...")
        
        # Verify model directory exists
        if not os.path.exists(MODEL_DIR):
            raise FileNotFoundError(f"Model directory not found: {MODEL_DIR}")
        
        # Verify 25ml subdirectory exists
        water_level_dir = os.path.join(MODEL_DIR, '25ml')
        if not os.path.exists(water_level_dir):
            raise FileNotFoundError(
                f"25ml model directory not found at: {water_level_dir}\n"
                f"Please ensure you have trained models for 25ml water level"
            )
        
        results = test_25ml_models(TEST_DATA_PATH, MODEL_DIR)
        ...
        print(f"2. Check model directory contains 25ml subfolder: {MODEL_DIR}")
        print("3. Ensure all required .pkl files are present in 25ml folder")
        print("4. Confirm your test data contains 25ml samples")